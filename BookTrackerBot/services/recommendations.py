...
class RecommendationSystem:
    ...
    def get_recommendations(self, user_id: int) -> List[Dict]:
        ...
        try:
            ...
            # Исключаем книги, которые уже есть у пользователя
            user_book_ids = {book['id'] for book in user_books}
            filtered_recs = [
                book for book in unique_recs 
                if book.get('id') not in user_book_ids
            ]
    ...
    def _get_content_based_recommendations(self, user_id: int) -> List[Dict]:
        ...
        try:
            ...
            for idx in sorted_indices[:10]:  # Топ-10 похожих книг
                if avg_similarities[idx] > 0.1:  # Минимальный порог схожести
